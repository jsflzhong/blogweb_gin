...
func UploadPost(c *gin.Context) {
	...
	fileExt := filepath.Ext(fileHeader.Filename) //取文件后缀名的API
	...
	err = os.MkdirAll(fileDir, os.ModePerm) //works on windows system? - yes.
	if err != nil {
		responseErr(c, err)
		return
	}
	...
	//上一行拼接后的路径的反斜杠是不对的:/static\upload\img\2020\3\29\1585463001-1582696150(1).png
	//需要都改成"/"
	filePathStr = filepath.ToSlash(filePathStr)
	log.Println("[UploadPost]Ready to upload file, path:", filePathStr)
